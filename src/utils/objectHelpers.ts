...
export const isNullValue = (value: any): boolean => value === undefined || value === null;
...
const setTestResult = (
  ...
  data: any,
  expected?: any,
  devMsgFailure?: string,
  userMsgFailure?: string,
  devMsgSuccess?: string,
  userMessageSuccess?: string
): ITestDataResult => {
  const msg: IMessage = {
  ...
  switch (criterion) {
    ...
    case CriteriaTest.StringContains:
      return setTestResult(
        criterion,
        !isNullOrEmpty(dataStr) && data.toString().includes(expected),
        data,
        expected,
        devMsgFailure,
        userMsgFailure,
    ...
    case CriteriaTest.LessThanOrEqualTo:
      return setTestResult(
        criterion,
        !isNullOrEmpty(dataStr) && Number(data) <= Number(expected),
        data,
        expected,
        devMsgFailure,
        userMsgFailure,
...
function* deepKeys(t: any, pre: any = []): Generator<any, void, unknown> {
  ...
  else if (Object(t) === t) for (const [k, v] of Object.entries(t)) yield* deepKeys(v, [...pre, k]);
  else yield pre.join(".");
}
